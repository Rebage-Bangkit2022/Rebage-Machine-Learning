...
class SSDMobileNetV2FpnKerasFeatureExtractor(
    ...
    def build(self, input_shape):
        full_mobilenet_v2 = mobilenet_v2.mobilenet_v2(
            batchnorm_training=(
                self._is_training and not self._freeze_batchnorm),
        ...
        for layer_idx in [4, 7, 14]:
            ...
            outputs.append(full_mobilenet_v2.get_layer(
        ...
        # Todo: Saving summary to file

        def myprint(s):
            with open('modelsummary.txt', 'w') as f:
                full_mobilenet_v2.summary(print_fn=lambda x: f.write(x + '\n'))

        full_mobilenet_v2.summary(print_fn=myprint)
        self.built = True
