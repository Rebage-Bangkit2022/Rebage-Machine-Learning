...
def keypoint_proto_to_params(kp_config, keypoint_map_dict):
    ...
    keypoint_std_dev = [keypoint_std_dev_dict[label]
                        for label in keypoint_labels]
    if kp_config.HasField('heatmap_head_params'):
        heatmap_head_num_filters = list(
            kp_config.heatmap_head_params.num_filters)
        heatmap_head_kernel_sizes = list(
            kp_config.heatmap_head_params.kernel_sizes)
    else:
        heatmap_head_num_filters = [256]
        heatmap_head_kernel_sizes = [3]
    if kp_config.HasField('offset_head_params'):
        offset_head_num_filters = list(
            kp_config.offset_head_params.num_filters)
        offset_head_kernel_sizes = list(
            kp_config.offset_head_params.kernel_sizes)
    else:
        offset_head_num_filters = [256]
        offset_head_kernel_sizes = [3]
    if kp_config.HasField('regress_head_params'):
        regress_head_num_filters = list(
            kp_config.regress_head_params.num_filters)
...
def object_detection_proto_to_params(od_config):
    ...
    if od_config.HasField('scale_head_params'):
        ...
        scale_head_kernel_sizes = list(
            od_config.scale_head_params.kernel_sizes)
    else:
        scale_head_num_filters = [256]
        scale_head_kernel_sizes = [3]
    if od_config.HasField('offset_head_params'):
        offset_head_num_filters = list(
            od_config.offset_head_params.num_filters)
        offset_head_kernel_sizes = list(
            od_config.offset_head_params.kernel_sizes)
    else:
        offset_head_num_filters = [256]
        offset_head_kernel_sizes = [3]
...
def object_center_proto_to_params(oc_config):
    ...
    if oc_config.keypoint_weights_for_center:
        keypoint_weights_for_center = list(
            oc_config.keypoint_weights_for_center)

    if oc_config.HasField('center_head_params'):
        center_head_num_filters = list(
            oc_config.center_head_params.num_filters)
        center_head_kernel_sizes = list(
...
def mask_proto_to_params(mask_config):
    ...
    if mask_config.HasField('mask_head_params'):
        ...
        mask_head_kernel_sizes = list(
            mask_config.mask_head_params.kernel_sizes)
    else:
